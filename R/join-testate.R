join_testate <- function(y) {
  ...
  y[, names(y) == "difluc"] <-  y[, names(y) == "difluc"] + y[, names(y) == "diflei"] +
    y[, names(y) == "difovi"] + y[, names(y) == "difrub"]
  y <- subset(y, select=-c(diflei, difovi, difrub))
  ...
  y[, names(y) == "eugtub"] <- y[, names(y) == "eugtub"] + y[, names(y) == "eugrot"]
  y <- subset(y, select=-c(eugrot))
  ...
  y[, names(y) == "nebwai"] <- y[, names(y) == "nebwai"] + y[, names(y) == "nebtub"]
  y <- subset(y, select=-c(nebtub))

  # y[, names(y) == "cortri"] <- y[, names(y) == "cortri"] + y[, names(y) == "cordub"]
  # y <- subset(y, select=-c(cordub))

  y[, names(y) == "triarc"] <- y[, names(y) == "triarc"] + y[, names(y) == "trimin"]
  y <- subset(y, select=-c(trimin))
