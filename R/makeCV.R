...
makeCV <- function (
  ...
  if (model_name=="MVGP") {
    ...
    # Rcpp::sourceCpp(here("mcmc", "mcmc-dirichlet-multinomial-mvgp.cpp"))
    out <- mcmc(
      BayesComposition::mcmcRcppDMMVGPMultiplicative(
        y_train,
        X_train,
        # y_test,
        params,
    ...
    preds <- BayesComposition::predictRcppDMMVGPMultiplicative(
      y_test,
      mean(X_train),
      var(X_train),
      min(X_train),
    ...
    ## predict the missing covariates
    # Rcpp::sourceCpp(here("functions", "makeCRPS.cpp"))
    CRPS <- makeCRPS(preds$X, X_test,
                     params$n_mcmc/params$n_thin)
    X_mean <- apply(preds$X, 2, mean)
    MSPE <- (X_mean - X_test)^2
    MAE  <- abs(apply(preds$X, 2, median) -  X_test)
    X_025 <- apply(preds$X, 2, quantile, prob = 0.025)
    X_975 <- apply(preds$X, 2, quantile, prob = 0.975)
    ...
    out <- mcmc(
      BayesComposition::mcmcRcppDMBasis(
        y_train,
        X_train,
        # y_test,
        params,
    ...
    samples <- BayesComposition::extract_compositional_samples(out)
    preds <- BayesComposition::predictRcppDMBasis(
      y_test,
      mean(X_train),
      var(X_train),
      min(X_train),
    ...
    ## Fit BUMMER model
    out <- mcmc(
      BayesComposition::mcmcRcppDMBummer(
        y_train,
        X_train,
        # y_test,
        params,
    ...
    samples <- BayesComposition::extract_compositional_samples(out)
    preds <- BayesComposition::predictRcppDMBummer(
      y_test,
      mean(X_train),
      var(X_train),
      min(X_train),
