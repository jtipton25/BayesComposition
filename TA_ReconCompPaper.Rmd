...
zeros_idx <- which(colSums(y_prop) == 0)
if (length(zeros_idx) > 0) {
  modWAPLS <- rioja::WAPLS(y_prop[, - zeros_idx], X, npls=5)
  predWAPLS <- predict(modWAPLS, y_recon_prop[, - zeros_idx], npls=5,
                       sse=TRUE, nboot=1000)
} else {
  ## no data to subset
  modWAPLS <- rioja::WAPLS(y_prop, X, npls=5)
  predWAPLS <- predict(modWAPLS, y_recon_prop, npls=5,
                       sse=TRUE, nboot=1000)      
}
...
caribou.df$Model <- factor(caribou.df$Model, levels = c("MVGP", "Bummer", "WA", "WAPLS", "MLRC", "MAT", "RF"))
