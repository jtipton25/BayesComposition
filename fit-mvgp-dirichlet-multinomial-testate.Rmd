...
save_file <- "mvgp-dm-testate.RData"
...
if (file.exists(paste0(save_directory, save_file))) {
  ...
  out <- fit_compositional_data(y=y, X=X, params=params, 
                                ...
                                progress_file = "mvgp-dm-testate.txt", 
...
caribou.df <- data.frame(
  Covariate=c(X_ci),
  Observation=factor(rep((1:N_pred), each=4*n_samples*0.95)), 
  Model=rep(c("MVGP", "WA", "MLRC", "MAT"), each=n_samples*0.95)
)
