...
```{r stan-model}
if (file.exists(here("mvgp-test", "fit-bummer-stan-mean.RData"))) {
  ## Load MCMC run
  load(here("mvgp-test", "fit-bummer-stan-mean.RData"))
} else {
  ## Long running MCMC 
  library(rstan)
  n_mcmc <- 500
  ...
  fit = stan(file=here("bummer-dm-mean.stan"), iter=n_mcmc,
             verbose=FALSE, data=dat, chains=4, init=init_fun,
             control=list(adapt_delta=0.99, stepsize=0.01, max_treedepth=15))
  save(fit, file=here("mvgp-test", "fit-bummer-stan-mean.RData"))
}
```
...
invisible(capture.output(
  mcmcplot(s, parms=c("mu", "sigma2", "alpha", "a", "gamma"), 
         dir=here("mvgp-test", "stan-bummer-diagnostics"),
         filename="stan-bummer-mean-diagnostics", extension="html", random=20)))
...
## join testatate species to be consistent with fitted data
...
for (k in 1:n_iter) {
  ...
  for (ii in 1:n_grid) {
    alpha_pred[k, ii, ] <- e$gamma[k, ] + e$a[k, ] * exp( - (e$mu[k, ] - X_grid[ii])^2 / e$sigma2[k, ])
    }
  for (i in 1:N_pred) {
