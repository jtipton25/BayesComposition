...
if (file.exists(paste0(save_directory, "other-models-cv.RData"))) {
  ...
  for (i in 1:10) {
    ...
    pred_sd_WA <- c(pred_sd_WA, sqrt(predWA$v1.boot[, 1]))
    ...
    pred_sd_MLRC <- c(pred_sd_MLRC, sqrt(predMLRC$v1.boot[, 1]))
    ...
    pred_sd_MAT <- c(pred_sd_MAT, sqrt(predMAT$v1.boot[, 2]))
...
abline(lm(apply(X_pred, 2, mean) ~ X[sample_idx]), col="red")
...
segments(x0=X[sample_idx], y0=pred_mu_WA - 2 * pred_sd_WA,
         ...
         col=adjustcolor("black", alpha.f=0.25))
abline(lm(pred_mu_WA ~ X[sample_idx]), col="red")
...
segments(x0=X[sample_idx], y0=pred_mu_MLRC - 2 * pred_sd_MLRC,
         ...
         col=adjustcolor("black", alpha.f=0.25))
abline(lm(pred_mu_MLRC ~ X[sample_idx]), col="red")
...
segments(x0=X[sample_idx], y0=pred_mu_MAT - 2 * pred_sd_MAT,
         ...
         col=adjustcolor("black", alpha.f=0.25))
abline(lm(pred_mu_MAT ~ X[sample_idx]), col="red")
...
segments(x0=X[sample_idx], y0=apply(preds_rf, 1, quantile, prob=0.025), 
         ...
         col=adjustcolor("black", alpha.f=0.25))
abline(lm(apply(preds_rf, 1, mean) ~ X[sample_idx]), col="red")
...
segments(x0=X[sample_idx], y0=apply(X_pred_bummer, 2, quantile, prob=0.025), 
         ...
         col=adjustcolor("black", alpha.f=0.25))
abline(lm(apply(X_pred_bummer, 2, mean) ~ X[sample_idx]), col="red")
