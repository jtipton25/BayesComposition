...
if (file.exists(paste0(save_directory, "other-models-cv.RData"))) {
  ...
  for (i in 1:10) {
    ...
    zeros_idx <- which(apply(y_prop_train, 1, sum) == 0)
    ...
    zeros_idx <- which(apply(y_prop_train, 1, sum) == 0)
    if (length(zeros_idx) > 0) {
      modWAPLS <- rioja::WAPLS(y_prop_train[, - zeros_idx], X_train, npls=5)
      predWAPLS <- predict(modWAPLS, y_prop_test[, - zeros_idx], 
                           npls=5, sse=TRUE, nboot=1000)
    } else {
      ## no data to subset
      modWAPLS <- rioja::WAPLS(y_prop_train, X_train, npls=5)
      predWAPLS <- predict(modWAPLS, y_prop_test, npls=5, sse=TRUE, nboot=1000)      
    }
    
    pred_mu_WAPLS <- c(pred_mu_WAPLS, predWAPLS$fit[, 5])
    pred_sd_WAPLS <- c(pred_sd_WAPLS, sqrt(predWAPLS$v1.boot[, 5]^2 + 
                                             predWAPLS$v2.boot[5]^2))
  ...
  save(pred_mu_WA, pred_sd_WA, pred_mu_WAPLS, pred_sd_WAPLS,
       pred_mu_MLRC, pred_sd_MLRC, pred_mu_MAT, pred_sd_MAT, preds_rf, 
...
if (file.exists(paste0(save_directory, "bummer-cv-predictions.RData"))) {
  ...
  for (i in 1:10) {
    ...
    X_grid <- seq(from=min(X_center) - 0.125*sd(X_center), 
                  to=max(X_center) + 0.125*sd(X_center), length=n_grid)
...
```{r} 
png(file="cv-plot1.png", width=9, height=6, units="in", res=400)
...
## sort the data to be in the same order as the predictions
plot(pred_mu_WAPLS ~ X[sample_idx], ylab="predicted mean", xlab="Observed", 
     main="Cross-validation of WAPLS model", xlim=c(-40, 80), ylim=c(-40, 80))
## Notice most of the confidence intervals cover the red linear regression line 
## i.e. the 95% CI is not close to 95%
segments(x0=X[sample_idx], y0=pred_mu_WAPLS - 2 * pred_sd_WAPLS,
         x1=X[sample_idx], y1=pred_mu_WAPLS + 2 * pred_sd_WAPLS,
         col=adjustcolor("black", alpha.f=0.25))
abline(a=0, b=1, col="red")
abline(lm(pred_mu_WAPLS ~ X[sample_idx]), col="blue")
# summary(lm(pred_mu_WAPLS ~ X[sample_idx]))
...
\begin{figure}
\includegraphics[width=1.0\linewidth]{cv-plot1.png}
\end{figure}

```{r}
coverage <- matrix(0, N, 7)
MSPE <- matrix(0, N, 7)
MAE <- matrix(0, N, 7)
CRPS <- matrix(0, N, 7)
...
for (i in 1:N) {
  ...
  MSPE[i, 4] <- (X[sample_idx][i] - pred_mu_WAPLS[i])^2
  MSPE[i, 5] <- (X[sample_idx][i] - pred_mu_MLRC[i])^2
  MSPE[i, 6] <- (X[sample_idx][i] - pred_mu_MAT[i])^2
  MSPE[i, 7] <- (X[sample_idx][i] - mean(preds_rf[i, ]))^2
  ...
  MAE[i, 4] <- abs(X[sample_idx][i] - pred_mu_WAPLS[i])
  MAE[i, 5] <- abs(X[sample_idx][i] - pred_mu_MLRC[i])
  MAE[i, 6] <- abs(X[sample_idx][i] - pred_mu_MAT[i])
  MAE[i, 7] <- abs(X[sample_idx][i] - median(preds_rf[i, ]))
  ...
  coverage[i, 4] <- (X[sample_idx][i] > (pred_mu_WAPLS[i] - 2 * pred_sd_WAPLS[i])) && (X[sample_idx][i] < (pred_mu_WAPLS[i] + 2 * pred_sd_WAPLS[i]))
  coverage[i, 5] <- (X[sample_idx][i] > (pred_mu_MLRC[i] - 2 * pred_sd_MLRC[i])) && (X[sample_idx][i] < (pred_mu_MLRC[i] + 2 * pred_sd_MLRC[i]))
  coverage[i, 6] <- (X[sample_idx][i] > (pred_mu_MAT[i] - 2 * pred_sd_MAT[i])) && (X[sample_idx][i] < (pred_mu_MAT[i] + 2 * pred_sd_MAT[i]))
  coverage[i, 7] <- X[sample_idx][i] > quantile(preds_rf[i, ], prob=0.025) &&
...
CRPS[, 4] <- makeCRPSGauss(pred_mu_WAPLS, pred_sd_WAPLS, X[sample_idx])
CRPS[, 5] <- makeCRPSGauss(pred_mu_MLRC, pred_sd_MLRC, X[sample_idx])
CRPS[, 6] <- makeCRPSGauss(pred_mu_MAT, pred_sd_MAT, X[sample_idx])
CRPS[, 7] <- makeCRPS(t(preds_rf), X[sample_idx], dim(preds_rf)[2])

models <- c("MVGP", "BUMMER", "WA", "WAPLS", "MLRC", "MAT", "RF")
...
library(xtable)
print(xtable(results), file="cv-results.tex")
```
