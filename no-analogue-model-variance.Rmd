...
```{r}
library(dplyr)
modMATDist <- MAT(y_prop, X, k=30, lean=FALSE)
screeplot(modMATDist)
modMATDist <- MAT(y_prop, X, k=10, lean=FALSE)
...
# analog_idx <- which(apply(modMATDist$dist.n, 1, mean)  > 0.5)
...
modMATDistTrain <- MAT(y_train_prop, X_train, k=10, lean=FALSE)
predMATDist <- predict(modMATDistTrain, y_test_prop, k=10, sse=TRUE, n.boot=1000)
...
save_file <- paste0("dm-testate-no-analog-variance.RData")
progress_directory <- "./mvgp-test/"
progress_file <- paste0("mvgp-dm-testate-no-analog-variance.txt")
```
...
if (file.exists(paste0(save_directory, save_file))) {
  ...
  parallelChains <- function (n_chains) {
    Rcpp::sourceCpp('~/mvgp/mcmc/mcmc-dm-basis-variance-spline-softmax.cpp')
...
gamma_post <- samples$gamma 
mu_beta_post <- samples$mu_beta
Sigma_beta_post <- samples$Sigma_beta
mu_gamma_post <- samples$mu_gamma
Sigma_gamma_post <- samples$Sigma_gamma
```
...
save_directory <- "./mvgp-test/"
save_file <- paste0("dm-testate-no-analog-variance-booth.RData")
progress_directory <- "./mvgp-test/"
progress_file <- paste0("mvgp-dm-testate-no-analog-variance-booth.txt")
```
...
if (file.exists(paste0(save_directory, save_file))) {
  ...
  Rhat <- make_gelman_rubin(out)
  png(file=here("mvgp-test", "diagnostics",
                paste0("Rhat-no-analog-variance-booth.png")),
      width=6, height=6, units="in", res=400)
  layout(matrix(1:4, 2, 2))
  hist(Rhat[grepl("beta", names(Rhat))], main = "Rhat for beta")
  hist(Rhat[grepl("gamma", names(Rhat))], main = "Rhat for gamma")
...
if (file.exists(paste0(save_directory, "other-models-no-analog-booth.RData"))) {
  load(file=paste0(save_directory, "other-models-no-analog-booth.RData"))
} else {
  y_train_prop <- as.matrix(y_train_prop)
  ...
  save(pred_mu_WA, pred_sd_WA, pred_mu_MLRC, pred_sd_MLRC, 
       pred_mu_MAT, pred_sd_MAT, preds_rf, 
       file= paste0(save_directory, "other-models-no-analog-booth.RData"))
}
```
...
if (file.exists(paste0(save_directory, "bummer-no-analog-booth.RData"))) {
  ## Load MCMC run
  load(paste0(save_directory, "bummer-no-analog-booth.RData"))
} else {
  ## Long running MCMC
  library(rstan)
  n_mcmc <- 500
  ...
  fit = stan(file="~/BayesComposition/bummer-dm.stan", iter=n_mcmc,
             verbose=FALSE, data=dat, chains=4, init=init_fun,
             control=list(adapt_delta=0.99, stepsize=0.01, max_treedepth=15))
  save(fit, file=paste0(save_directory, "bummer-no-analog-booth.RData"))
}
```
...
X_pred_bummer <- c()
if (file.exists(paste0(save_directory, "bummer-no-analog-predictions-booth.RData"))) {
  load(file=paste0(save_directory, "bummer-no-analog-predictions-booth.RData"))
} else {
  ## load the data
  load(paste0(save_directory, "bummer-no-analog-booth.RData"))
  ## extract samples
...
save(X_pred_bummer,
     file=paste0(save_directory, "bummer-no-analog-predictions-booth.RData"))
```
...
save_directory <- "./mvgp-test/"
save_file <- paste0("dm-pollen-no-analog-variance-booth.RData")
progress_directory <- "./mvgp-test/"
progress_file <- paste0("mvgp-dm-pollen-no-analog-variance-booth.txt")
```
...
if (file.exists(paste0(save_directory, save_file))) {
  ...
  Rhat <- make_gelman_rubin(out)
  png(file=here("mvgp-test", "diagnostics",
                paste0("Rhat-no-analog-variance-pollen.png")),
      width=6, height=6, units="in", res=400)
  layout(matrix(1:4, 2, 2))
  hist(Rhat[grepl("beta", names(Rhat))], main = "Rhat for beta")
  hist(Rhat[grepl("gamma", names(Rhat))], main = "Rhat for gamma")
...
if (file.exists(paste0(save_directory, "other-models-no-analog-pollen.RData"))) {
  load(file=paste0(save_directory, "other-models-no-analog-pollen.RData"))
} else {
  ...
  save(pred_mu_WA, pred_sd_WA, pred_mu_MLRC, pred_sd_MLRC, 
       pred_mu_MAT, pred_sd_MAT, preds_rf, 
       file= paste0(save_directory, "other-models-no-analog-pollen.RData"))
}
```
...
# X_center <- (X_train-mu_X) / sd_X
X_center <- X[-analog_idx]
if (file.exists(paste0(save_directory, "bummer-no-analog-pollen.RData"))) {
  ## Load MCMC run
  load(paste0(save_directory, "bummer-no-analog-pollen.RData"))
} else {
  ## Long running MCMC
  library(rstan)
  n_mcmc <- 500
  ...
  fit = stan(file="~/BayesComposition/bummer-dm.stan", iter=n_mcmc,
             verbose=FALSE, data=dat, chains=4, init=init_fun,
             control=list(adapt_delta=0.99, stepsize=0.01, max_treedepth=15))
  save(fit, file=paste0(save_directory, "bummer-no-analog-pollen.RData"))
}
```
...
X_pred_bummer <- c()
if (file.exists(paste0(save_directory, "bummer-no-analog-predictions-pollen.RData"))) {
  load(file=paste0(save_directory, "bummer-no-analog-predictions-pollen.RData"))
} else {
  ## load the data
  load(paste0(save_directory, "bummer-no-analog-pollen.RData"))
  ## extract samples
...
save(X_pred_bummer,
     file=paste0(save_directory, "bummer-no-analog-predictions-pollen.RData"))
```
