...
// [[Rcpp::export]]
List predictRcppDMMVGPMultiplicativeAdditive (const arma::mat& Y_pred, const double mu_X,
    ...
    for (int j=0; j<n_rep; j++) {
      ...
      if (sample_X) {
        for (int i=0; i<N_pred; i++) {
          ...
          Rcpp::List ess_out = ess_X_multiplicative_additive(X_pred(i), X_prior, mu_X, X_knots,
      ...
      if (sample_epsilon) {
        for (int i=0; i<N_pred; i++) {
          ...
          Rcpp::List ess_out = ess_epsilon_multiplicative_additive(epsilon_pred.row(i), epsilon_prior,
