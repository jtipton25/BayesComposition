...
Rcpp::List ess_epsilon_additive (const arma::rowvec& epsilon_current,
  ...
  while (test) {
    ...
    arma::rowvec alpha_proposal = exp(mu_current + zeta_current + epsilon_proposal);
...
Rcpp::List ess_X_additive (const double& X_current, const double& X_prior,
  ...
  while (test) {
    ...
    arma::rowvec alpha_proposal = exp(mu_current + zeta_proposal + epsilon_current);
...
// [[Rcpp::export]]
List predictRcppDMMVGPAdditive (const arma::mat& Y_pred, const double mu_X,
  ...
  arma::mat xi_additive_fit = as<mat>(samples["xi_additive"]);
  arma::vec xi_additive = xi_additive_fit.row(0).t();
  arma::mat tau2_additive_fit = as<mat>(samples["tau2_additive"]);
  arma::vec tau2_additive = tau2_additive_fit.row(0).t();
  // arma::cube R_additive_fit = as<cube>(samples["R_additive"]);
  // arma::mat R_additive = R_additive_fit.subcube(0, 0, 0, 0, d-1, d-1);
  ...
  arma::mat alpha_pred = exp(mu_mat + zeta_pred + epsilon_pred);
  ...
  for (int k=0; k<n_samples; k++) {
    ...
    for (int j=0; j<n_rep; j++) {
      ...
      xi_additive = xi_additive_fit.row(k).t();
      tau2_additive = tau2_additive_fit.row(k).t();
      // R_additive = R_additive_fit.subcube(k, 0, 0, k, d-1, d-1);
      R_tau_additive = R_tau_additive_fit.subcube(k, 0, 0, k, d-1, d-1);
      zeta_pred = Z * eta_star * R_tau;
      alpha_pred = exp(mu_mat + zeta_pred + epsilon_pred);

      if (sample_X) {
        for (int i=0; i<N_pred; i++) {
          double X_prior = R::rnorm(0.0, s_X);
          // double X_prior = R::rnorm(mu_X, s_X);
          Rcpp::List ess_out = ess_X_additive(X_pred(i), X_prior, mu_X, X_knots,
                                              Y_pred.row(i), mu.t(), eta_star, epsilon_pred,
      ...
      if (sample_epsilon) {
        for (int i=0; i<N_pred; i++) {
          ...
          Rcpp::List ess_out = ess_epsilon_additive(epsilon_pred.row(i), epsilon_prior,
                                                    Y_pred.row(i), mu.t(), zeta_pred,
                                                    alpha_pred.row(i),
                                                    d, count_pred(i),
                                                    file_name, corr_function);
