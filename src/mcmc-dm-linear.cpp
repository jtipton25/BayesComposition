...
// [[Rcpp::export]]
List mcmcRcppDMLinear (const arma::mat& Y,
                       const arma::vec& X,
                       List params,
  ...
  arma::vec zero_2(2, arma::fill::zeros);
  ...
  arma::mat Sigma_beta_0(2, 2, arma::fill::eye);
  Sigma_beta_0 *= 1.0;
  ...
  arma::mat Xbs = join_rows(ones_N, X);
  ...
  arma::cube beta_save(n_save, 2, d, arma::fill::zeros);
  arma::mat mu_beta_save(n_save, 2, arma::fill::zeros);
  arma::cube Sigma_beta_save(n_save, 2, 2, arma::fill::zeros);
  ...
  arma::cube beta_batch(50, 2, d, arma::fill::zeros);
  arma::cube Sigma_beta_tune(2, 2, d, arma::fill::zeros);
  arma::cube Sigma_beta_tune_chol(2, 2, d, arma::fill::zeros);
  for(int j=0; j<d; j++) {
  ...
  for (int k=0; k<n_adapt; k++) {
    ...
    if (sample_beta) {
      ...
      // update tuning
      beta_batch.subcube(k % 50, 0, 0, k % 50, 2-1, d-1) = beta;
      if ((k+1) % 50 == 0){
